...
const DEFAULT_API_ADDRESS =
  "https://huk0nxydai.execute-api.us-east-1.amazonaws.com/dev";
let API_ADDRESS =
  window.localStorage.getItem("zillow_commenter_api_address") ||
  DEFAULT_API_ADDRESS;
...
console.log(
  "Saved current tab ID:",
  CURRENT_TABS[0] ? CURRENT_TABS[0].id : "failed to save",
);
    ...
    getNewUserId((result, error = null) => {
      ...
      if (result) {
        ...
        window.localStorage.setItem(
          "zillow_commenter_user_id",
          parsedResult.user_id,
        );
      } else {
...
function displayCommentFetchError(error) {
  ...
  li.innerHTML =
    "<strong>Error fetching comments.</strong> Please try again later.";
...
function displayComments(result, error = null) {
  ...
  comments.forEach((comment) => {
    ...
    if (
      ...
      !isNaN(Number(comment.timestamp))
    ) {
...
async function handleCommentSubmission(event) {
  ...
  postComment(commentObj)
    ...
    .catch((error) => {
      console.log(
        "Got error when posting comment to " +
          `${API_URL}/comments` +
          ": " +
          error,
      );
...
function getCommentsByListingId(listingIdAndType, callbackFunc) {
  ...
  fetch(
    `${API_URL}/comments/${listingIdAndType.listingID}?listing_type=${listingIdAndType.listingType}`,
    requestOptions,
  )
    .then((response) => response.text())
    .then((result) => callbackFunc(result))
...
function initializeCommentInputValidation() {
  ...
  document
    ...
    .addEventListener("keyup", function validateCommentInput() {
...
async function getListingTitle() {
  ...
  if (chrome.runtime.lastError) {
    ...
    throw new Error(
      "Content script not available:" + chrome.runtime.lastError.message,
    );
  }
